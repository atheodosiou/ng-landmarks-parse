...
let LoginComponent = class LoginComponent {
    constructor(authService, formBuilder, router, toastService, store) {
    ...
    onSubmit(value) {
        this.authService.login(value.username, value.password).then((user) => {
            this.store.dispatch(new src_store_actions_auth_actions__WEBPACK_IMPORTED_MODULE_7__["AuthLoggedInAction"]());
            this.toastService.show(`Welcome ${user.getUsername()}!`, { classname: 'bg-success text-light', delay: 1000 });
            this.router.navigate(['/home']);
        }).catch(error => {
            this.store.dispatch(new src_store_actions_auth_actions__WEBPACK_IMPORTED_MODULE_7__["AuthLoggedOutAction"]());
